...
const DEFAULT_SPACING={ ratingsTopGapPx: 8 }, DEFAULT_PRIORITIES={
...
}, COLOR_THRESHOLDS={ green:75, orange:50, red:0 };
const COLOR_VALUES={ green:'limegreen', orange:'orange', red:'crimson' }, CACHE_DURATION=7*24*60*60*1000;
const NS='mdbl_', ICON_BASE='https://raw.githubusercontent.com/xroguel1ke/jellyfin_ratings/refs/heads/main/assets/icons';
const LOGO={
...
}, __CFG__=(typeof window!=='undefined' && window.MDBL_CFG) ? window.MDBL_CFG : {};
const ENABLE_SOURCES=Object.assign({}, DEFAULT_ENABLE_SOURCES, __CFG__.sources   || {}), DISPLAY=Object.assign({}, DEFAULT_DISPLAY,        __CFG__.display   || {});
const SPACING=Object.assign({}, DEFAULT_SPACING,        __CFG__.spacing   || {}), RATING_PRIORITY=Object.assign({}, DEFAULT_PRIORITIES,     __CFG__.priorities|| {});
const INJ_KEYS=(window.MDBL_KEYS||{}), LS_KEYS_JSON=localStorage.getItem(`${NS}keys`);
const LS_KEYS=LS_KEYS_JSON ? (JSON.parse(LS_KEYS_JSON)||{}) : {}, MDBLIST_API_KEY=String(INJ_KEYS.MDBLIST || LS_KEYS.MDBLIST || 'hehfnbo9y8blfyqm1d37ikubl');
if (typeof GM_xmlhttpRequest==='undefined'){
  const PROXIES=['https://api.allorigins.win/raw?url=','https://api.codetabs.com/v1/proxy?quest='], DIRECT=['api.mdblist.com','graphql.anilist.co','query.wikidata.org','api.themoviedb.org'];
  window.GM_xmlhttpRequest=({method='GET',url,headers={},data,onload,onerror})=>{
    const isDirect=DIRECT.some(d=>url.includes(d)), proxy=PROXIES[Math.floor(Math.random()*PROXIES.length)];
    const sep=url.includes('?')?'&':'?', final=isDirect?url:(proxy+encodeURIComponent(url+sep+`_=${Date.now()}`));
...
function ensureInlineBadge(){
  ...
  const before=findYearChip(primary)||primary.firstChild, badge=document.createElement('span');
...
function scanLinks(){
  ...
  [...document.querySelectorAll('a.emby-button[href*="themoviedb.org/"]')].forEach(a=>{
    ...
    document.querySelectorAll('.itemMiscInfo.itemMiscInfo-primary').forEach(b=>{
      ...
      const justify=DISPLAY.align==='center'?'center':DISPLAY.align==='left'?'flex-start':'flex-end', paddingRight=DISPLAY.align==='right'?'6px':'0';
...
function fetchRTDirectLink(imdbId, cb){
  const key=`${NS}rturl_${imdbId}`, cache=localStorage.getItem(key);
  ...
  GM_xmlhttpRequest({
    ...
    onload:r=>{
      try{
        const val=JSON.parse(r.responseText).results.bindings[0]?.rtid?.value || '', url=val ? ('https://www.rottentomatoes.com/' + val.replace(/^https?:\/\/(?:www\.)?rottentomatoes\.com\
  ...
  GM_xmlhttpRequest({
    ...
    onload:r=>{
      ...
      d.ratings?.forEach(rr=>{
        const s=(rr.source||'').toLowerCase(), v=rr.value, cnt=rr.votes||rr.count||rr.reviewCount||rr.ratingCount;
        if (s.includes('imdb') && ENABLE_SOURCES.imdb)
          ...
          const seg=(container.dataset.type==='show')?'tv':'movie', link=slug?`https://www.metacritic.com/${seg}/${slug}`:`https://www.metacritic.com/search/all/${encodeURIComponent(title)}/results`;
...
function fetchRT_HTMLFallback(imdbId, container){
  const key=`${NS}rt_${imdbId}`, cache=localStorage.getItem(key);
  if(cache){
        ...
        GM_xmlhttpRequest({
          ...
          onload:rr=>{
            try{
              ...
              if(!m) return;
              const d=JSON.parse(m[1]), critic=Util.num(d.criticsScore?.score);
              const cCount=(typeof d.criticsScore?.reviewCount==='number')?d.criticsScore.reviewCount:undefined, audience=Util.num(d.audienceScore?.score);
              const aCount=(typeof d.audienceScore?.ratingCount==='number')?d.audienceScore.ratingCount:undefined, scores={critic,audience,link:url,cCount,aCount};
...
(function settingsMenu(){
  const PREFS_KEY=`${NS}prefs`, LS_KEYS=`${NS}keys`;
  ...
  const ICON={ imdb:LOGO.imdb, tmdb:LOGO.tmdb, trakt:LOGO.trakt, letterboxd:LOGO.letterboxd,
    ...
    roger_ebert:LOGO.roger, anilist:LOGO.anilist, myanimelist:LOGO.myanimelist }, LABEL={ imdb:'IMDb', tmdb:'TMDb', trakt:'Trakt', letterboxd:'Letterboxd',
...
function mdblUpdateSwatches(){
    ...
    const kWrap=qs('#mdbl-sec-keys');
    const injKey=getInjectorKey(); const stored=getStoredKeys().MDBLIST||'';
    const value=injKey?injKey:(stored||''), readonly=injKey?'readonly':'';
    ...
    const sWrap=qs('#mdbl-sec-sources');
    ...
    const dWrap=qs('#mdbl-sec-display');
    dWrap.innerHTML=`
    ...
    const _ygInput=qs('#th_yg'), _labelTop=qs('#label_top_tier');
    if(_ygInput && _labelTop){
      const upd=()=>{
        const v=parseInt(_ygInput.value||'79',10), x=isNaN(v)?80:(v+1);
