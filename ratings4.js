// ==UserScript==
// @name         Jellyfin Ratings (v7.1.1 — Icons & Menu Fix)
// @namespace    https://mdblist.com
// @version      7.1.1
// @description  Unified ratings. Fixes missing icons due to mapping errors. Fixes Menu opening. Includes Caching & Live Preview.
// @match        *://*/*
// @grant        GM_xmlhttpRequest
// ==/UserScript>

console.log('[Jellyfin Ratings] v7.1.1 loading...');
...
// FIX: Direct mapping of config keys to filenames
const LOGO = {
...
// Global Click Listener (Menu Trigger)
...
function renderRatings(container, data, imdbId) {
    ...
    if (data.ratings) {
        data.ratings.forEach(r => {
            ...
            if (s.includes('imdb')) add('imdb', v, `https://www.imdb.com/title/${imdbId}/`, c, 'IMDb');
                ...
                add('rotten_tomatoes_critic', v, '#', c, 'RT Critic');
            }
            else if (s.includes('popcorn') || s.includes('audience')) {
                add('rotten_tomatoes_audience', v, '#', c, 'RT Audience');
            }
    ...
    // AniList/MAL fallbacks could be added here if needed, skipping for brevity to ensure stability
...
(function initMenu() {
    const css = `
        ...
        .mdbl-src-row { background: rgba(255,255,255,0.03); padding: 6px 10px; border-radius: 8px; margin-bottom: 6px; cursor: grab; }
    ...
    panel.addEventListener('mousedown', (e) => {
        ...
        if (e.target.closest('.sec')) return; // Only drag on header
        isDrag = true;
...
function renderMenuContent(panel) {
    ...
    bind('#d_cnum', (e) => { CFG.display.colorNumbers = e.target.checked; refreshDomElements(); });
    bind('#d_cicon', (e) => { CFG.display.colorIcons = e.target.checked; refreshDomElements(); });
    bind('#d_pct', (e) => { CFG.display.showPercentSymbol = e.target.checked; refreshDomElements(); });
    ...
    panel.querySelectorAll('.mdbl-src-row').forEach(row => {
        ...
        row.addEventListener('drop', e => {
            ...
            if (dragSrc !== row) {
                ...
                let all = [...list.querySelectorAll('.mdbl-src-row')];
