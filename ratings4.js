// ==UserScript==
// @name         Jellyfin Ratings (v7.2.0 — Stable Live Preview)
// @namespace    https://mdblist.com
// @version      7.2.0
// @description  Restored v7.0.1 design. Fixed Live Preview for %, Sorting (all items), and Colors.
// @match        *://*/*
// @grant        GM_xmlhttpRequest
// ==/UserScript>

console.log('[Jellyfin Ratings] v7.2.0 loading...');
...
(function () {
    ...
    function appendRating(container, logo, val, title, key, link, count, kind) {
        ...
        img.dataset.score = r; // Store score for live updates
        ...
        if (d.ratings) {
            d.ratings.forEach(rr => {
                ...
                if (s.includes('imdb') && ENABLE_SOURCES.imdb)
                    ...
                    appendRating(container, LOGO.metacritic_critic, v, 'Metacritic (Critic)', 'metacritic_critic', link, cnt, 'Reviews');
                }
                    ...
                    appendRating(container, LOGO.roger_ebert, v, 'Roger Ebert', 'roger_ebert', `https://www.rogerebert.com/reviews/${slug}`, cnt);
            });
        }
        ...
        if (hasMDBL_RT) {
            fetchRTDirectLink(imdbId, (rtUrl) => {
                ...
                if (ENABLE_SOURCES.rotten_tomatoes_critic && rtCriticVal != null)
                    appendRating(container, LOGO.rotten_tomatoes_critic, rtCriticVal, 'RT Critic', 'rotten_tomatoes_critic', link, rtCriticCnt, 'Reviews');
                if (ENABLE_SOURCES.rotten_tomatoes_audience && rtAudienceVal != null)
                    appendRating(container, LOGO.rotten_tomatoes_audience, rtAudienceVal, 'RT Audience', 'rotten_tomatoes_audience', link, rtAudienceCnt, 'Votes');
            });
        }
    ...
    function fetchRT_HTMLFallback(imdbId, container) {
        ...
        function addRT(c, s) {
            if (Util.ok(s.critic) && ENABLE_SOURCES.rotten_tomatoes_critic) appendRating(c, LOGO.rotten_tomatoes_critic, s.critic, 'RT Critic', 'rotten_tomatoes_critic', s.link || '#', s.cCount, 'Reviews');
            if (Util.ok(s.audience) && ENABLE_SOURCES.rotten_tomatoes_audience) appendRating(c, LOGO.rotten_tomatoes_audience, s.audience, 'RT Audience', 'rotten_tomatoes_audience', s.link || '#', s.aCount, 'Votes');
        }
    }
...
(function settingsMenu() {
  ...
  `;
    ...
    function updateLiveDisplay() {
        ...
        const showPercent = panel.querySelector('#d_showPercent').checked;
        ...
        document.querySelectorAll('.mdbl-rating-item').forEach(el => {
            ...
            if(!isNaN(score)) {
                ...
                // Text (%)
                span.textContent = showPercent ? `${score}%` : `${score}`;
            }
        });
    }
    
    function updateLiveSourceVisibility(key, isVisible) {
        document.querySelectorAll(`.mdbl-rating-item[data-source="${key}"]`).forEach(el => {
            el.style.display = isVisible ? 'inline-flex' : 'none';
        });
    }
    
    function updateLivePriorities() {
        document.querySelectorAll('.mdblist-rating-container').forEach(container => {
             [...container.children].forEach(child => {
                 const k = child.dataset.source;
                 if(k && RATING_PRIORITY[k]) child.style.order = RATING_PRIORITY[k];
             });
        });
    }
    ...
    function enableDnD(container) {
        ...
        ['drop', 'dragend'].forEach(evt => container.addEventListener(evt, () => { 
            if (dragging) {
                ...
                // Update Priority Object Live
                const rows = [...container.querySelectorAll('.mdbl-source')];
                rows.forEach((r, i) => {
                    RATING_PRIORITY[r.dataset.k] = i + 1;
                });
    ...
    function render() {
        ...
        // Bind Live Color Updates
        ['#th_red', '#th_orange', '#th_yg', '#col_red', '#col_orange', '#col_yg', '#col_mg', '#d_colorNumbers', '#d_colorIcons', '#d_showPercent'].forEach(sel => {
             const el = panel.querySelector(sel);
             if(el) {
                 el.addEventListener('input', updateLiveDisplay);
                 el.addEventListener('change', updateLiveDisplay);
             }
        });
    ...
    panel.querySelector('#mdbl-btn-reset').addEventListener('click', () => {
        ...
        // Also re-apply order and visibility
        updateLivePriorities();
        Object.keys(ENABLE_SOURCES).forEach(k => updateLiveSourceVisibility(k, true));
