...
const COLOR_VALUES     = { deep:'seagreen', light:'lightgreen', orange:'orange', red:'crimson' };
...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  // coloring (per-number/icon toggles only)
  {
    const cutR=+DISPLAY.colorRedMax, cutO=+DISPLAY.colorOrangeMax, cutL=+DISPLAY.colorLightMax;
    const col = (r<=cutR) ? COLOR_VALUES.red : (r<=cutO) ? COLOR_VALUES.orange : (r<=cutL) ? COLOR_VALUES.light : COLOR_VALUES.deep;
...
(function settingsMenu(){
  ...
  function render(){
    ...
    kWrap.innerHTML=`${(value&&value.length)?`<div id="mdbl-key-box" class="mdbl-source"><div class="mdbl-subtle">MDBList API key is stored</div></div>`:`<div id="mdbl-key-box" class="mdbl-source"><input type="text" id="mdbl-key-mdb" placeholder="MDBList API key" value=""></div>`}`;
    ...
    dWrap.innerHTML=`
      ...
      <div class="mdbl-row"><span>Red max (%)</span><input type="number" id="d_cut_red" min="0" max="100" value="${DISPLAY.colorRedMax}"></div>
      <div class="mdbl-row"><span>Orange max (%)</span><input type="number" id="d_cut_orange" min="0" max="100" value="${DISPLAY.colorOrangeMax}"></div>
      <div class="mdbl-row"><span>Light green max (%)</span><input type="number" id="d_cut_light" min="0" max="100" value="${DISPLAY.colorLightMax}"></div>
  ...
  panel.querySelector('#mdbl-btn-save').addEventListener('click', ()=>{
    ...
    prefs.display.showPercentSymbol = panel.querySelector('#d_showPercent').checked;prefs.display.align             = panel.querySelector('#d_align').value;
    prefs.display.endsAtFormat      = panel.querySelector('#d_endsFmt').value;

    
    // color bands (validate & save)
    let _r=parseInt(panel.querySelector('#d_cut_red')?.value,10);
    let _o=parseInt(panel.querySelector('#d_cut_orange')?.value,10);
    let _l=parseInt(panel.querySelector('#d_cut_light')?.value,10);
    _r=isFinite(_r)?Math.max(0,Math.min(100,_r)):50;
    _o=isFinite(_o)?Math.max(0,Math.min(100,_o)):69;
    _l=isFinite(_l)?Math.max(0,Math.min(100,_l)):79;
    if(_o<=_r) _o=_r+1; if(_l<=_o) _l=_o+1; if(_l>100) _l=100;
    prefs.display.colorRedMax=_r; prefs.display.colorOrangeMax=_o; prefs.display.colorLightMax=_l;
