...
/* === MDBList Ratings — UI Consistency Patch (v12 full) === */
(function(){
  const STYLE_ID = 'mdbl-ui-v12-style';

  function injectBaseCSS(){
    if (document.getElementById(STYLE_ID)) return;
    const css = `
      /* Dark select-like styling for the three % inputs */
      #mdbl-panel #th_red,
      ...
      /* Keep color-band labels from wrapping */
      #mdbl-panel .mdbl-grid label{ white-space:nowrap; }
    `;
    const st = document.createElement('style'); st.id = STYLE_ID; st.textContent = css;
    (document.head||document.documentElement).appendChild(st);
  }

  function num(v){ try{ const n = parseFloat(v||''); return (isFinite(n)&&n>0) ? n : NaN; }catch{ return NaN; } }

  function stylePercentInputs(panel){
    ...
    ['th_red','th_orange','th_yg'].forEach(id=>{
      ...
      el.style.setProperty('width','56px','important');      /* narrow so the line stays single */
      el.style.setProperty('padding','0 10px','important');  /* same feel as selects */
      el.style.setProperty('text-align','center','important');
      // Ensure the special class (for any existing select styles)
      try{ el.classList.add('mdbl-select'); }catch{}
    });
  }

  function matchRowBoxHeights(panel){
    const alignSel = panel.querySelector('#d_align');
    const endsSel  = panel.querySelector('#d_endsFmt');
    if(!alignSel || !endsSel) return;
    const alignRow = alignSel.closest('.mdbl-row');
    const endsRow  = endsSel.closest('.mdbl-row');
    if(!alignRow || !endsRow) return;

    // Get a reference row in the Display section above (e.g., "Show %")
    const section = alignRow.closest('.mdbl-section');
    let ref = null;
    if(section){
      const rows = Array.from(section.querySelectorAll('.mdbl-row'));
      const idx = rows.indexOf(alignRow);
      for(let i=idx-1;i>=0;i--){
        const r = rows[i];
        if(r && !r.classList.contains('wide')){ ref=r; break; }
      }
      if(!ref) ref = rows[0] || null;
    }
    if(!ref) ref = panel.querySelector('.mdbl-row');
    if(!ref) return;

    const cs = getComputedStyle(ref);
    const mh = num(cs.minHeight);
    const hh = num(cs.height);
    const targetH = isFinite(mh) && mh>0 ? mh : (isFinite(hh) && hh>0 ? hh : 48);

    function applyRowBox(row){
      row.style.setProperty('min-height', targetH+'px', 'important');
      ...
      const border = (isFinite(bw) && bw>0) ? (cs.borderTopStyle && cs.borderTopStyle!=='none' ? `1px ${cs.borderTopStyle} ${cs.borderTopColor}` : cs.border) : '1px solid rgba(255,255,255,0.06)';
      row.style.setProperty('border', border, 'important');
      row.style.setProperty('background', cs.backgroundColor || 'transparent', 'important');
    }

    applyRowBox(alignRow);
    applyRowBox(endsRow);
  }

  function applyAll(root){
    const panel = (root||document).querySelector('#mdbl-panel');
    if(!panel) return;
    injectBaseCSS();
    stylePercentInputs(panel);
    matchRowBoxHeights(panel);
  }

  try{ applyAll(document); }catch{}
  try{
    const obs = new MutationObserver((ms)=>{
      for(const m of ms){
        for(const n of m.addedNodes){
          if(n && n.nodeType===1 && (n.id==='mdbl-panel' || (n.querySelector && n.querySelector('#mdbl-panel')))){
            setTimeout(()=>applyAll(n),0);
          }
        }
      }
    });
...
/* === MDBList Ratings — Align/Ends strict row height (v13) === */
(function(){
  function applyStrict(){
    const panel = document.querySelector('#mdbl-panel'); if(!panel) return;
    // pick a reference row above Align
    const alignSel = panel.querySelector('#d_align');
    const endsSel  = panel.querySelector('#d_endsFmt');
    const alignRow = alignSel && alignSel.closest ? alignSel.closest('.mdbl-row') : null;
    const endsRow  = endsSel  && endsSel.closest  ? endsSel.closest('.mdbl-row')  : null;
    if(!alignRow || !endsRow) return;

    // choose a nearby non-wide row as reference (e.g., "Show %")
    const section = alignRow.closest('.mdbl-section');
    let ref = null;
    if(section){
      const rows = Array.from(section.querySelectorAll('.mdbl-row'));
    ...
    const target = Math.max(0, Math.round(rect.height)) || 48;

    [alignRow, endsRow].forEach(row=>{
      try{
        row.style.setProperty('height', target+'px', 'important');
        row.style.setProperty('min-height', target+'px', 'important');
        row.style.setProperty('box-sizing','border-box','important');
        // keep padding/border consistent with existing (don’t override if already set by earlier patch)
        if(!row.style.paddingTop){ row.style.setProperty('padding-top','10px','important'); }
        if(!row.style.paddingBottom){ row.style.setProperty('padding-bottom','10px','important'); }
        if(!row.style.paddingLeft){ row.style.setProperty('padding-left','12px','important'); }
        if(!row.style.paddingRight){ row.style.setProperty('padding-right','12px','important'); }
        if(!row.style.borderRadius){ row.style.setProperty('border-radius','12px','important'); }
        if(!row.style.border){ row.style.setProperty('border','1px solid rgba(255,255,255,0.06)','important'); }
      }catch{}
    });
  }
  try{ applyStrict(); }catch{}
  try{
    const obs = new MutationObserver(applyStrict);
    obs.observe(document.documentElement,{childList:true,subtree:true});
  }catch{}
})();
...
/* === MDBList Ratings — Row content fit fix (v14) === */
(function(){
  function fixFit(){
    const panel = document.querySelector('#mdbl-panel'); if(!panel) return;
    const alignSel = panel.querySelector('#d_align');
    ...
    // Reference control height from palette select so inner controls fit comfortably
    let ctlH = 32;
    try{
      const refSel = panel.querySelector('#col_red') || panel.querySelector('select.mdbl-select');
      const cs = refSel ? getComputedStyle(refSel) : null;
      const hh = cs ? parseFloat(cs.height) : NaN;
      if (isFinite(hh) && hh>0) ctlH = Math.round(hh);
    }catch{}

    function relaxRow(row){
      if(!row) return;
      try{
        // Let the row grow if content needs more space (no fixed height)
        row.style.removeProperty('height');
        row.style.setProperty('min-height', (ctlH + 12) + 'px', 'important'); // control + a bit of padding
    ...
    function sizeSelect(sel){
      if(!sel) return;
      try{
        sel.style.setProperty('height', ctlH+'px', 'important');
        sel.style.setProperty('min-height', ctlH+'px', 'important');
      }catch{}
    }

    relaxRow(alignRow); relaxRow(endsRow);
    sizeSelect(alignSel); sizeSelect(endsSel);
  }
  try{ fixFit(); }catch{}
  try{
    const obs = new MutationObserver(fixFit);
    obs.observe(document.documentElement,{childList:true,subtree:true});
  }catch{}
})();
