...
const LOGO = {
  ...
  metacritic_user:metacritic_user:`${ICON_BASE}/mus2.png`,
  average:`data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='64' height='64' viewBox='0 0 64 64'><rect rx='10' ry='10' x='2' y='2' width='60' height='60' fill='%232b2f36'/><text x='50%25' y='54%25' font-family='system-ui,Segoe UI,Roboto,Arial' font-size='28' fill='%23eaeaea' text-anchor='middle'>AVG</text></svg>`,
};
  ...
  [...document.querySelectorAll('a.emby-button[href*="themoviedb.org/"]')].forEach(a=>{
    ...
    document.querySelectorAll('.itemMiscInfo.itemMiscInfo-primary').forEach(b=>{
      ...
      Object.assign(div.dataset,{type, tmdbId, mdblFetched:'0'}); ref.insertAdjacentElement('afterend',div);
       ...
       try{ if(ENABLE_SOURCES.average) MDBL_ENSURE_AVG(div); }catch{} try{
...
function appendRating(container, logo, val, title, key, link, count, kind){
  ...
  // number — also opens settings
  const s=document.createElement('span'); s.textContent=disp; s.style='font-size:1em;vertical-align:middle;cursor:pointer;';
...
// Sources
    ...
    dWrap.innerHTML=`
      ...
      <div class="mdbl-row wide">
        ...
        <div class="grid-right" style="gap:10px; justify-content:flex-end;">
          <input type="range" id="d_posX" min="0" max="100" step="1" value="${(DISPLAY.posX ?? 0)}" style="width:140px">
          <input type="number" id="d_posX_num" class="mdbl-num" min="0" max="100" value="${(DISPLAY.posX ?? 0)}"> %
        </div>
      </div>
      <div class="mdbl-row wide">
        <span>Y position</span>
        <div class="grid-right" style="gap:10px; justify-content:flex-end;">
          <input type="range" id="d_posY" min="0" max="100" step="1" value="${(DISPLAY.posY ?? 0)}" style="width:140px">
          <input type="number" id="d_posY_num" class="mdbl-num" min="0" max="100" value="${(DISPLAY.posY ?? 0)}"> %
        </div>
      </div>
     
      
      <div class="mdbl-row wide">
        <span>Enable 24h format</span>
        <div class="grid-right" style="justify-content:flex-end">
          <input type="checkbox" id="d_ends24h" ${DISPLAY.endsAtFormat==='24h'?'checked':''}>
        </div>
      </div>
    ...
    try{
      ...
      function sync(setDisp=true){
        const xv = clamp01(xR?.value ?? xN?.value ?? 0);
        const yv = clamp01(yR?.value ?? yN?.value ?? 0);
        if(xR) xR.value=xv; if(xN) xN.value=xv; if(yR) yR.value=yv; if(yN) yN.value=yv;
        if(setDisp){ DISPLAY.posX = xv; DISPLAY.posY = yv; }
        if(window.MDBL_APPLY_POS) window.MDBL_APPLY_POS(xv,yv);
      }
  ...
  panel.querySelector('#mdbl-btn-save').addEventListener('click', ()=>{
    ...
    const clamp01=(v)=>{ v=parseInt(v||'0',10); if(!isFinite(v)) v=0; return Math.max(0,Math.min(100,v)); };
const __prevX = DISPLAY.posX, __prevY = DISPLAY.posY;
const __x = clamp01(panel.querySelector('#d_posX')?.value ?? panel.querySelector('#d_posX_num')?.value ?? 0);
const __y = clamp01(panel.querySelector('#d_posY')?.value ?? panel.querySelector('#d_posY_num')?.value ?? 0);
prefs.display.posX = (__prevX==null && __prevY==null && __x===0 && __y===0) ? null : __x;
prefs.display.posY = (__prevX==null && __prevY==null && __x===0 && __y===0) ? null : __y;
