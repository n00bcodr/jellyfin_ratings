...
function ensureInlineBadge(){
  const primary=findPrimaryRow(); if(!primary) return false;
  const ratingValue=readAndHideOriginalBadge(); if(!ratingValue) return false;
  const before=findYearChip(primary)||primary.firstElementChild||primary.firstChild;
  let badge=primary.querySelector('#mdblistInlineParental');
  if(!badge){
    badge=document.createElement('span');
    badge.id='mdblistInlineParental';
    Object.assign(badge.style,{display:'inline-flex',alignItems:'center',gap:'6px',opacity:'0.9'});
  }
  ...
  // success if badge is the first element now
  return !!(primary.firstElementChild===badge);
}
...
function ensureEndsAtInline(){
  const primary=findPrimaryRow(); if(!primary) return false;
  const r=findRuntimeNode(primary); const runtimeNode=r&&r.node; const minutes=r&&r.minutes; if(!runtimeNode||!minutes) return false;
  const end=new Date(Date.now()+minutes*60000);
  const fmt=(DISPLAY.endsAtFormat==='12h')?(()=>{ let h=end.getHours(); const m=Util.pad(end.getMinutes()), suf=h>=12?'PM':'AM'; h=h%12||12; return `${h}:${m} ${suf}`; })():`${Util.pad(end.getHours())}:${Util.pad(end.getMinutes())}`;
  let span=primary.querySelector('#customEndsAt');
  if(!span){ span=document.createElement('span'); span.id='customEndsAt'; Object.assign(span.style,{marginLeft:'6px',whiteSpace:'nowrap'}); }
  span.textContent=`Ends at ${fmt}`;
  if(runtimeNode && runtimeNode.parentNode){
    if(runtimeNode.insertAdjacentElement){ runtimeNode.insertAdjacentElement('afterend', span); }
    else { (runtimeNode.nextSibling)?runtimeNode.parentNode.insertBefore(span,runtimeNode.nextSibling):runtimeNode.parentNode.appendChild(span); }
  } else if(!span.parentNode) {
    primary.appendChild(span);
  }
  // success if span sits right after runtime
  return !!(span.previousElementSibling===runtimeNode);
}
...
// Minimal bounded retry to catch late DOM chips without global observers
function mdblTry(fn, tries=12, delay=120){
  let left=tries;
  const tick=()=>{ try{ if(fn()) return; }catch
    // Ensure correct first-line order even if runtime chip appears a bit late
    mdblTry(()=>{ const ok1=ensureInlineBadge(); const ok2=ensureEndsAtInline(); return ok1&&ok2; }, 15, 100);
{}; if(--left<=0) return; setTimeout(tick, delay); };
  tick();
}
function updateAll(){
  try{
