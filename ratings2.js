...
/* Clone parental rating to start — and make it open Settings */
function ensureInlineBadge(){
  ...
  // Use existing badge if present; otherwise create it
  ...
  if (!badge){
    ...
    Object.assign(badge.style, {
      ...
      color:'var(--theme-text-color,#ddd)', marginRight:'10px',
      whiteSpace:'nowrap', flex:'0 0 auto', verticalAlign:'middle'
    });
    (before && before.parentNode)
      ? before.parentNode.insertBefore(badge, before)
      : primary.insertBefore(badge, primary.firstChild);
  }

  // Make badge act as a button to open the Settings panel
  badge.title = 'Open settings';
  badge.style.cursor = 'pointer';
  badge.setAttribute('role', 'button');
  badge.setAttribute('tabindex', '0');

  if (!badge.dataset.mdblSettings){
    badge.dataset.mdblSettings = '1';
    const open = (e) => {
      e.preventDefault();
      e.stopPropagation();
...
// -- Compatibility: ensure parental badge opens Settings even if not replaced --
(function(){
  function wire(badge){
    if (!badge || badge.dataset.mdblSettings) return;
    badge.dataset.mdblSettings = '1';
    badge.title = 'Open settings';
    badge.style.cursor = 'pointer';
    badge.setAttribute('role','button');
  ...
  // try once now
  wire(document.getElementById('mdblistInlineParental'));

  // observe for future inserts (SPA navigations)
  const mo = new MutationObserver(() => {
    const b = document.getElementById('mdblistInlineParental');
    if (b) wire(b);
  });
