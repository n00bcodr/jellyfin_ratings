...
(function(){
  ...
  function addStyles(){
    ...
    const css = `
      ...
      /* Backup: ensure wide rows have the same box height */
      #mdbl-panel .mdbl-row.wide{ min-height:48px !important; }
`;
...
(function(){
  ...
  function patchPanel(panel){
    ...
    // Match the "box" height of Align and Ends rows to other rows
    try{
      const alignSel = panel.querySelector('#d_align');
      const endsSel  = panel.querySelector('#d_endsFmt');
      function styleRowBox(el){
        try{
          const row = el && el.closest ? el.closest('.mdbl-row') : null;
          if(row){
            row.style.setProperty('min-height','48px','important');
            row.style.setProperty('padding','10px 12px','important');
            row.style.setProperty('border-radius','12px','important');
            row.style.setProperty('border','1px solid rgba(255,255,255,0.06)','important');
          }
        }catch{}
      }
      styleRowBox(alignSel);
      styleRowBox(endsSel);
    }catch{}
  ...
  try{
    if(!document.getElementById('mdbl-inline-fallback-style')){
      ...
      /* Keep color-band label + input on one line */
      #mdbl-panel .mdbl-grid label{ white-space:nowrap; }
      #mdbl-panel .mdbl-grid .grid-row{ align-items:center; }
  ...
  try{
    ...
    [n1,n2,n3].forEach(n=>{ if(n){ n.style.setProperty('width','56px','important'); n.style.setProperty('text-align','right'); n.style.setProperty('display','inline-block'); } });
  }catch{}
})();
