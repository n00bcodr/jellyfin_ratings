...
function ensureEndsAtInline(){
  const primary=findPrimaryRow(); if(!primary) return;
  const res=findRuntimeNode(primary); const node=res&&res.node; const minutes=res&&res.minutes; if(!node||!minutes) return;
  const end=new Date(Date.now()+minutes*60000);
  const timeStr=(DISPLAY.endsAtFormat==='12h')?(()=>{ let h=end.getHours(); const m=Util.pad(end.getMinutes()), suf=h>=12?'PM':'AM'; h=h%12||12; return `${h}:${m} ${suf}`; })():`${Util.pad(end.getHours())}:${Util.pad(end.getMinutes())}`;
  const content=`Ends at ${timeStr}`;
  let span=primary.querySelector('#customEndsAt');
  if(!span){ span=document.createElement('span'); span.id='customEndsAt'; Object.assign(span.style,{marginLeft:'6px',color:'inherit',opacity:'0.9',fontSize:'inherit',fontWeight:'inherit',whiteSpace:'nowrap',display:'inline'}); }
  span.textContent=content;
  if(node && node.parentNode){
    if(node.insertAdjacentElement){ node.insertAdjacentElement('afterend', span); }
    else { (node.nextSibling)?node.parentNode.insertBefore(span,node.nextSibling):node.parentNode.appendChild(span); }
