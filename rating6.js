...
/* ==== MDBList sliders + average + 24h checkbox — append-only safe patch ==== */
(function(){
  const POS_KEY = 'mdbl_pos_v1';
  const AVG_ICON = "data:image/svg+xml;utf8,"+encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64">
      <rect rx="10" ry="10" x="2" y="2" width="60" height="60" fill="#2b2f36"/>
      <text x="50%" y="54%" font-family="system-ui,Segoe UI,Roboto,Arial" font-size="28" fill="#eaeaea" text-anchor="middle">AVG</text>
    </svg>`
  );

  function readPos(){
    try{ const o=JSON.parse(localStorage.getItem(POS_KEY)||'{}'); if(typeof o.x==='number'&&typeof o.y==='number') return o; }catch{}
    return null;
  }
  function writePos(x,y){
    try{ localStorage.setItem(POS_KEY, JSON.stringify({x,y})); }catch{}
  }
  function clearPos(){ try{ localStorage.removeItem(POS_KEY); }catch{} }

  function applyStoredPos(){
    const st = readPos(); if(!st) return;
    document.querySelectorAll('.mdblist-rating-container').forEach(div=>{
      try{
        div.style.position='fixed';
        div.style.left='0'; div.style.top='0';
        div.style.transform=`translate(${st.x}vw, ${st.y}vh)`;
  ...
  try{
    const mo = new MutationObserver(()=>applyStoredPos());
    mo.observe(document.documentElement,{childList:true,subtree:true});
  }catch{}

  // Average helpers (independent of ENABLE_SOURCES/ICON/LABEL)
  function ensureAverage(container){
    ...
    const img=document.createElement('img'); img.src=AVG_ICON; img.alt='Average'; img.style='height:1.3em;vertical-align:middle;cursor:pointer;';
    img.title='Average of all sources that returned a rating';
    a.appendChild(img);
    a.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); window.MDBL_OPEN_SETTINGS&&window.MDBL_OPEN_SETTINGS(); });
    ...
    s.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); window.MDBL_OPEN_SETTINGS&&window.MDBL_OPEN_SETTINGS(); });
    wrap.append(a,s);
  ...
  function updateAverage(container){
    ...
    if(!wrap){ ensureAverage(container); }
    const span = (wrap||container.querySelector('[data-source="average"]'))?.querySelector('span');
    if(!span) return;
    if(vals.length){
      const r = Math.round(vals.reduce((a,b)=>a+b,0)/vals.length);
      span.textContent = (window.DISPLAY && DISPLAY.showPercentSymbol) ? `${r}%` : `${r}`;
      try{
        if(window.DISPLAY && (DISPLAY.colorNumbers||DISPLAY.colorIcons)){
          const img = (wrap||container.querySelector('[data-source="average"]')).querySelector('img');
          const col = getRatingColor(DISPLAY.colorBands, DISPLAY.colorChoice, r);
          if(DISPLAY.colorNumbers) span.style.color = col;
          if(DISPLAY.colorIcons)   img.style.filter = `drop-shadow(0 0 3px ${col})`;
        }
      }catch{}
    }else{
      span.textContent='';
      try{ (wrap||container.querySelector('[data-source="average"]')).querySelector('img')?.style.removeProperty('filter'); }catch{}
    }
  }

  // Patch appendRating to add clicks + collect for average
  (function waitForAppend(){
    if (typeof window.appendRating === 'function'){
      const __append = window.appendRating;
      window.appendRating = function(container, logo, val, title, key, link, count, kind){
        ...
        try{
          ...
          if(added){
            ...
            if(a){ a.href='#'; a.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); window.MDBL_OPEN_SETTINGS&&window.MDBL_OPEN_SETTINGS(); }; }
            if(s){ s.style.cursor='pointer'; s.onclick=(e)=>{ e.preventDefault(); e.stopPropagation(); window.MDBL_OPEN_SETTINGS&&window.MDBL_OPEN_SETTINGS(); }; }
          }
          // collect normalized 0..100
          const n = Util && Util.normalize ? Util.round(Util.normalize(val,key)) : null;
          if(isFinite(n)){ container.__vals = container.__vals||[]; container.__vals.push(n); }
          updateAverage(container);
        }catch{}
      };
    }else{
      setTimeout(waitForAppend, 80);
    }
  })();

  // When a container is added, ensure average badge + apply stored pos
  try{
    const mo2 = new MutationObserver((mutList)=>{
      for(const m of mutList){
        m.addedNodes && m.addedNodes.forEach(n=>{
          if(n.nodeType===1 && n.classList?.contains('mdblist-rating-container')){
            try{ ensureAverage(n); }catch{}
            try{ applyStoredPos(); }catch{}
          }
        });
      }
    });
  ...
  // Settings panel surgery: replace Align with X/Y sliders, replace Ends format with checkbox
  (function waitForPanel(){
    const panel=document.querySelector('#mdbl-panel');
    if(!panel){ return setTimeout(waitForPanel, 120); }
    if(panel.dataset._mdblEnhanced==='1') return;
    panel.dataset._mdblEnhanced='1';

    // Replace Ends at format dropdown with checkbox
    try{
      const sel = panel.querySelector('#d_endsFmt');
      if(sel){
        const row = sel.closest('.mdbl-row');
        const wrap = document.createElement('div');
        wrap.className='mdbl-row wide';
        wrap.innerHTML = '<span>Enable 24h format</span><div class="grid-right" style="justify-content:flex-end"><input type="checkbox" id="d_ends24h"></div>';
        row.parentNode.insertBefore(wrap,row);
        const cb = wrap.querySelector('#d_ends24h');
        cb.checked = (sel.value==='24h');
        cb.addEventListener('change', ()=>{ sel.value = cb.checked ? '24h' : '12h'; });
        row.style.display='none';
      }
    }catch{}

    // Sliders for X/Y in place of Align
    try{
      const align = panel.querySelector('#d_align');
      if(align){
        const row = align.closest('.mdbl-row.wide');
        const makeRow = (id,label)=>{
          const d=document.createElement('div'); d.className='mdbl-row wide';
          d.innerHTML = `<span>${label}</span>
            <div class="grid-right" style="gap:10px; justify-content:flex-end;">
              <input type="range" id="${id}" min="0" max="100" step="1" style="width:140px">
              <input type="number" id="${id}_num" class="mdbl-num" min="0" max="100"> %
            </div>`;
          return d;
        };
        const rX=makeRow('d_posX','X position'), rY=makeRow('d_posY','Y position');
        row.parentNode.insertBefore(rX,row);
        row.parentNode.insertBefore(rY,row);
        row.style.display='none';

        const pos = readPos() || (function(){
          const c=document.querySelector('.mdblist-rating-container');
          const rect=c?c.getBoundingClientRect():null;
          return {
            x: rect?Math.round(rect.left/window.innerWidth*100):0,
            y: rect?Math.round(rect.top /window.innerHeight*100):0
          };
        })();

        const xR=rX.querySelector('#d_posX'), xN=rX.querySelector('#d_posX_num');
        const yR=rY.querySelector('#d_posY'), yN=rY.querySelector('#d_posY_num');
        function clamp01(v){ v=parseInt(v||'0',10); if(!isFinite(v)) v=0; return Math.max(0,Math.min(100,v)); }
        function sync(setStore=true){
          const xv = clamp01(xR.value||xN.value||0);
          const yv = clamp01(yR.value||yN.value||0);
          xR.value=xv; xN.value=xv; yR.value=yv; yN.value=yv;
          document.querySelectorAll('.mdblist-rating-container').forEach(div=>{
            div.style.position='fixed'; div.style.left='0'; div.style.top='0';
            div.style.transform=`translate(${xv}vw, ${yv}vh)`; div.style.zIndex='99999'; div.style.pointerEvents='auto';
          });
          if(setStore) writePos(xv,yv);
        }
        xR.value=pos.x; xN.value=pos.x; yR.value=pos.y; yN.value=pos.y;
        xR.addEventListener('input', ()=>sync(true));
        xN.addEventListener('input', ()=>sync(true));
        yR.addEventListener('input', ()=>sync(true));
        yN.addEventListener('input', ()=>sync(true));

        // Reset clears stored pos
        const resetBtn = panel.querySelector('#mdbl-btn-reset');
        resetBtn && resetBtn.addEventListener('click', ()=>{ clearPos(); setTimeout(()=>location.reload(), 50); }, {once:true});
      }
    }catch{}
  })();

  // initial apply stored pos
  applyStoredPos();

  try{ console.log('[MDBList] sliders+avg patch active'); }catch{}
})();
